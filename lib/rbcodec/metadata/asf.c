                             ...
                             )
{
    ...
    const int utf8_bytes_to_use = *utf8bytes;
    ...
    while ((n = read(fd, utf16buf, MIN(sizeof(utf16buf) - 2, utf16bytes))) >= 2)
    {
        ...
        newutf8 = utf16decode(utf16buf, newutf8, n>>1, *utf8bytes, true);
        *utf8bytes = utf8_bytes_to_use - (newutf8 - *utf8);
        utf16bytes -= n;
        if (*utf8bytes <= 0)
            break;
    }
