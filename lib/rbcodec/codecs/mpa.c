...
static int get_file_pos(int newtime)
{
    ...
    if (id3->vbr) {
        ...
        if (id3->has_toc) {
            ...
            //LOGF("skip_from_curpos (s): %ld, skip_from_toc (s): %d, toc position (s): %u", skipms_from_curpos/1000, skipms_from_toc/1000, percent*pct_timestep/1000 );
            ...
            if (newtime < abs(skipms_from_curpos))
