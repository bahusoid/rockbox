...
const char * const sysfs_bat_tte =
"/sys/class/power_supply/battery/time_to_empty_now";

unsigned int hiby_power_get_battery_voltage(void)
{
    int battery_voltage;
...
unsigned int hiby_power_get_battery_capacity(void)
{
...
/* Get battery time to empty (now) */
unsigned int hiby_power_get_battery_tte(void)
{
    int battery_tte;
    sysfs_get_int(sysfs_bat_tte, &battery_tte);
