...
static inline bool is_handle_free(int handle)
{
    ...
    FOREACH_ALBUMART(i)
    {
        if (handle == last_folder_aa_hid[i])
            return false;
    }
    return true;
}
...
static void track_list_free_buf_info(struct track_buf_info *tbip)
{
    ...
    FOR_EACH_TRACK_INFO_HANDLE(i)
        if (is_handle_free(tbip->info.handle[i]))
...
static int load_album_art_from_path(char *path, struct bufopen_bitmap_data *user_data, bool is_current_track, int i)
{
    ...
    bool samePath = strcmp(last_folder_aa_path, path) == 0;
    if (samePath && last_folder_aa_hid[i] != 0)
        return last_folder_aa_hid[i];

    // To simplify logic a bit we keep track only for first AA path
...
static void audio_on_finish_load_track(int id3_hid)
{
    ...
    if (seek_on_finish_load_time > 0)
    {
        if (id3_hid == seek_on_finish_load_id3_hid)
            codec_seek(seek_on_finish_load_time);
